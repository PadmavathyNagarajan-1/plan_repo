                                  ...
                                  CREATE PROCEDURE [0ae85a19-0393-43da-b50e-2a91e0ca6c00].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                      ...
                                      SET @sql = N'
                                          CREATE TABLE [0ae85a19-0393-43da-b50e-2a91e0ca6c00].' + QUOTENAME(@TempTableName) + N' (
                                              id varchar(255),
                                              visualId int,
                                              name varchar(255),
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [0ae85a19-0393-43da-b50e-2a91e0ca6c00].data_input_row (id, name, rowMeta, visualRowConfigId, dimensionId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId,temp.dimensionId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [0ae85a19-0393-43da-b50e-2a91e0ca6c00].' + @TempTableName + ' as temp;';
