            ...
            CREATE PROCEDURE [ede3309b-972b-4b4a-a429-3b1fa37cf58a].[sp_planning_di_row_batch_update] 
            ...
            DECLARE @TempTableName NVARCHAR(128) = '#CR_UPDATE_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
              ...
              -- Insert data into the temporary table using dynamic SQL
                ...
                SET @sql = @sql + 'UPDATE [ede3309b-972b-4b4a-a429-3b1fa37cf58a].data_input_row 
                SET 
                    ...
                    updatedBy = @updatedBy,
                    updatedAt = @updatedAt
                FROM [ede3309b-972b-4b4a-a429-3b1fa37cf58a].data_input_row
                INNER JOIN [ede3309b-972b-4b4a-a429-3b1fa37cf58a].' +  @TempTableName + ' as temp on temp.id = data_input_row.id;';
