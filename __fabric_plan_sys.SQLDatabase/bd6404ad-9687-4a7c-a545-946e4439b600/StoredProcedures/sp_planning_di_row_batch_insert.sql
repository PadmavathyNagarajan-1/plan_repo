                                  ...
                                  CREATE PROCEDURE [bd6404ad-9687-4a7c-a545-946e4439b600].[sp_planning_di_row_batch_insert] 
                                  ...
                                  DECLARE @TempTableName NVARCHAR(128) = '#CR_INSERT_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
                                      ...
                                      SET @sql = N'
                                          CREATE TABLE [bd6404ad-9687-4a7c-a545-946e4439b600].' + QUOTENAME(@TempTableName) + N' (
                                              id varchar(255),
                                              visualId int,
                                              name varchar(255),
                                    ...
                                    -- Insert data into the temporary table using dynamic SQL
                                      ...
                                      SET @sql = @sql + 'INSERT INTO [bd6404ad-9687-4a7c-a545-946e4439b600].data_input_row (id, name, rowMeta, visualRowConfigId, dimensionId, rowPath,  visualId, createdBy, updatedBy, createdAt, updatedAt)
                                      SELECT temp.id, temp.name, temp.rowMeta, temp.visualRowConfigId,temp.dimensionId, temp.rowPath, temp.visualId, createdBy, updatedBy, createdAt, updatedAt FROM [bd6404ad-9687-4a7c-a545-946e4439b600].' + @TempTableName + ' as temp;';
