            ...
            CREATE PROCEDURE [bbd5af1b-fbc3-4e1e-b752-f1e75709f896].[sp_planning_di_row_batch_update] 
            ...
            DECLARE @TempTableName NVARCHAR(128) = '#CR_UPDATE_TEMP' + REPLACE(REPLACE(REPLACE(REPLACE(@currentTimeString, '-', ''), ':', ''),' ',''),'.','');
              ...
              -- Insert data into the temporary table using dynamic SQL
                ...
                SET @sql = @sql + 'UPDATE [bbd5af1b-fbc3-4e1e-b752-f1e75709f896].data_input_row 
                SET 
                    ...
                    updatedBy = @updatedBy,
                    updatedAt = @updatedAt
                FROM [bbd5af1b-fbc3-4e1e-b752-f1e75709f896].data_input_row
                INNER JOIN [bbd5af1b-fbc3-4e1e-b752-f1e75709f896].' +  @TempTableName + ' as temp on temp.id = data_input_row.id;';
